...
|---|---|---|
| `@consulta-dev/autofill` | Web Component e contrato público | Beta `v0.1.1` publicada |
| `@consulta-dev/qr-engine` | Interface de leitura de QR no navegador | Beta `v0.1.1` publicada |
| `apps/embed` | Aplicação hospedada no iframe | Beta `v0.1.1` publicada |
...
A beta [`v0.1.1`](https://github.com/consulta-dev-br/consulta-autofill/releases/tag/v0.1.1) está publicada no CDN e na GitHub Release. Os pacotes npm ainda aguardam a configuração de Trusted Publishing; até lá, integre uma versão exata pelo CDN. A API e o produto continuam em beta e não devem ser assumidos como estáveis.
